...
class CellPoseMetrics:
    ...
    def evaluate(self, predicted_masks: list, ground_truth_masks: list, metrics: list):
        if self.return_image_metrics:
            try:
                self.dataset_metric_dict, self.image_metric_dict, self.eval_metrics = self.compute_metrics(
                    predicted_masks,
                    ground_truth_masks,
                    metrics,
                    )
    ...
    def compute_metrics(self, prediction, ground_truth,
                        metrics=['f1_batch_mean', 'f1_batch_std', 'f1', 'mean_ap', 'jaccard', 'mean_aji', 'dice',
                                 'recall', 'precision']):
        # Calculate the evaluation metrics
        valid_metrics = ['f1_batch_mean', 'f1_batch_std', 'f1', 'mean_ap', 'jaccard', 'mean_aji', 'dice', 'recall',
                         'precision']
        ...
        for metric in metrics:
            if metric in valid_metrics:
                dataset_metric_dict[metric] = result_metrics[metric].astype('float64')[
                    0]  # convert len=1 numpy array to float64
            else:
